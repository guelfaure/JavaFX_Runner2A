...
public class GameScene extends Scene {
    ...
    public void update(long time){
        ...
        for (int u=1;u<this.nombrefoe-1;u++) {
            ...
            if (herorobot.isInvincible(time)==false && ( new Rectangle2D(this.listfoe.get(u).getX()+10, this.listfoe.get(u).getY()+20+250, 40, 75).intersects(rectangleHitboxHero)==true)){
                ...
                this.herorobot.invincibility = time+3100000000.0; //cette valeur permet dans la plupart du temps de ne pas prendre en compte le prochain ennemi
                ...
                switch (this.nombrecoeur){
                    ...
                    case 1:
                        ...
                        break;
                    case 0:
                        this.nombredevie.bellevue.setViewport(new Rectangle2D(200, 300, 1, 1));//on n'affiche plus aucun coeur
                        break;
                    case -1:
                        this.nombredevie.bellevue.setViewport(new Rectangle2D(300, 300, 1, 1));//toujours aucune vie mais ce coup ci c'est perdu!
    ...
    {public void handle(long time){
        if (time-tim1>=100_000_000) {       //100_000_000 en ns == durationbtw (temps entre chaques frags)
            tim1=time;
            herorobot.update();
            camera.update();
            update(time);
