...
//Guel FAURE 2G1TP5
//on creer une caméra qui suit le robot
    Camera(double x, double y, Hero hero){
        this.x = x;
        this.y = y;
        this.hero=hero;
        this.f = 52;
        this.m = 0.8;              //masse de la caméra
        this.k = (f*f*1.6)/4;     //force du ressort
        pastStates = new ArrayList(Collections.nCopies(4, 0.0));
        pastStates.set(0, x);
        pastStates.set(1, 0.0); //vitesse initiale en x de la cam
        pastStates.set(2, y);
        pastStates.set(3, 0.0); //vitesse initiale en y de la cam
        TimeBetweenUpdate = 0.016;//intervalle de temps entre chaque calcul de position
